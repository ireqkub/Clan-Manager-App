# Wolvesville Clan Manager (Prompt Specification)
...
To prevent Cross-Site Scripting (XSS) from malicious player names or flair strings:
* **Implementation:** All dynamic content (Usernames, Flairs, Logs, Ledger, Quests) must be sanitized before rendering.
* **Method:** Use React's default escaping for text. For any complex string parsing involving symbols, use a sanitization library (e.g., DOMPurify).
* **Logic:**
    * Strip or encode `<script>`, `onerror`, and `onclick` attributes from any API-returned strings.
    * Ensure the **"Flair Parsing"** logic only extracts expected characters (`©`, `G`, numbers, and specific emojis) to prevent injection of malicious code.

### 2. Secure Token Management
While `sessionStorage` provides basic isolation, the following security layers should be implemented:
* **In-Memory Fallback:** Prefer storing the token in a global state (e.g., React Context or Redux) during the session, using `sessionStorage` only as a secondary backup for page refreshes.
* **Encryption (Optional):** Encrypt the token using a simple AES-256 (client-side) before saving to `sessionStorage` to prevent plain-text visibility in browser developer tools.
* **Clearance:** Implement an explicit **Logout** button that calls `sessionStorage.clear()` and resets all internal application states.

### 3. Content Security Policy (CSP)
Define a strict policy to prevent unauthorized data exfiltration and enhance browser-level security:
* **Directive:** `default-src 'self';`
* **API Restriction:** `connect-src 'self' https://api.wolvesville.com;` (Strictly prevents the app from sending data to any unauthorized third-party domains).
* **Script Restriction:** Disallow `unsafe-inline` scripts to mitigate the impact of potential XSS vulnerabilities.
...
5. Don't have a bot token? [Create a bot](https://api-docs.wolvesville.com/#/?id=create-a-bot)
---
...
* **Flair Parsing Logic:**
    ...
    * Identify special emojis: `📙` (Gold), `📘` (Gem), `📕` (Opt-out), 🏆 (TOP 1 QUEST).
...
-  **Update TOP 1 Button:** for who have most xp in participants remove 🏆 for who already have and added (OR replace any emoji end with) to the TOP 1 QUEST.
-  If member who already have 🏆 and still have most xp in participants. Just leave it like that, don't do anything.
---
# Page 4: Quest Active
