...
import { AlertTriangle, Loader2, Filter, Users, Trophy, ExternalLink, RefreshCw } from 'lucide-react';
...
export default function QuestFeePage() {
  ...
  function analyzeFeeChanges() {
    ...
    for (const participant of lowXpParticipants) {
      ...
      // Keep existing emoji unless it's none, then add ⚠️ if negative
    ...
    if (changes.length === 0) {
      toast({ title: 'No Members', description: 'No members to apply fee to.' });
    } else {
      setPendingFeeChanges(changes);
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">{clanName} — Apply {FEE_AMOUNT}© fee to members below {XP_THRESHOLD.toLocaleString()} XP</p>
          </div>
